...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  while ($row = $result->fetch_assoc()) {
    ...
    //$template_codigo = $row['codigo_verificacion'];
    //$template_expira = $row['codigo_expiracion'];
  }
  ...
  if ($filas > 0):
    ...
    if ($template_acceso == 1) {
      ...
      //$_SESSION['template_codigo'] = $template_codigo;
      //$_SESSION['template_expira'] = $template_expira;
